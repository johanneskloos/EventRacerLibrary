...
void VarsInfo::findRaces(const ActionLog& actions, const SimpleDirectedGraph& graph) {
	...
	m_numChains = 0;
	if (graph_connectivity_algorithm == "CD") {
...
bool VarsInfo::shouldTimeout() {
	if (race_detection_timeout_seconds != 0) {
		bool result = (GetCurrentTimeMicros() - m_startTime) > race_detection_timeout_seconds * 1000000;
		if (result) {
			m_timedOut = true;
